/**
 * Swap'n'Serve Cup — tournament engine (Deno edge-function copy).
 *
 * Exact mirror of `src/lib/tournament.ts`, which is the source of truth and
 * carries the Vitest suite. Edge functions can't import from `src/`, so the
 * (zero-dependency) engine is duplicated here. Keep the two in sync.
 *
 * Flow: drawGroups → play group fixtures → computeStandings →
 * qualifiersFromGroups → buildBracket → recordKnockoutResult.
 */
